...
function ProjectManagerInsertLink() {
	...
	var counter = document.getElementById('counter_panel');
	...
	if (project.className.indexOf('current') != -1) {
		...
		var searchform = getCheckedValue(document.getElementById('searchform'));
		...
		if (projectId != 0)
			tagtext = "[project id=" + projectId + " template=" + template + cat + orderby + order + selections + searchform + limit +"]";
		else
			tinyMCEPopup.close();
	}
	...
	if (datasetform.className.indexOf('current') != -1) {
		...
		if (projectId != 0)
			tagtext = "[dataset_form project_id=" + projectId + template + "]";
		else
			tinyMCEPopup.close();
	}

	if (counter.className.indexOf('current') != -1) {
		var projectId = document.getElementById('counter_project').value;
		var field = document.getElementById('counter_field').value;
		var text = document.getElementById('counter_text').value;
		
		if (projectId != 0)
			tagtext = "[projectmanager_counter project_id=" + projectId + " field=" + field + " text='" + text + "']";
		else
			tinyMCEPopup.close();
	}
	...
	if (map.className.indexOf('current') != -1) {
		var projectId = document.getElementById('map_project').value;
		
		if (projectId != 0)
			tagtext = "[projectmanager_map project_id=" + projectId + "]";
		else
			tinyMCEPopup.close();
	}
